...
`````nCreate a SaaS landing page hero section with the following exact specifications:

Page Layout

The entire page is h-screen flex flex-col bg-background overflow-hidden â€” the Navbar + Hero fill exactly 100vh with no scroll.
...
Left: Logo text âœ¦ Nexora â€” text-xl font-semibold tracking-tight text-foreground
Right (hidden on mobile): Nav links "Home", "Pricing", "About", "Contact" â€” text-sm text-muted-foreground hover:text-foreground with gap-8
...
Text: "Now with GPT-5 support âœ¨"
mb-6
2. Headline
...
Content: The Future of Smarter Automation â€” the word "Smarter" renders in Instrument Serif italic
3. Subheadline
...
Text: "Automate your busywork with intelligent agents that learn, adapt, and executeâ€”so your team can focus on what matters most."
4. CTA Buttons

Framer Motion: fade up from y:16, duration 0.6s, delay 0.3s
mt-5 flex items-center gap-3
Primary button: rounded-full px-6 py-5 text-sm font-medium font-body â€” text "Book a demo"
...
Top bar: Logo "N" in rounded box + "Nexora" + chevron | Search bar with âŒ˜K shortcut | "Move Money" + bell + avatar "JB"
Sidebar (w-40): Items â€” Home (active), Tasks (badge "10"), Transactions, Payments (chevron), Cards, Capital, Accounts (chevron). Section "Workflows": Trake rutes, Payments, Notifications, Settings
Main content (bg-secondary/30):
Greeting: "Welcome, Jane" â€” text-sm font-semibold
Action buttons row: Send (primary/accent), Request, Transfer, Deposit, Pay Bill, Create Invoice â€” rounded-full pill buttons text-[10px], + "Customize" text
Two equal-width cards (flex-1 basis-0) side by side:
Balance card: "Mercury Balance" with checkmark, amount $8,450,190.32 (cents in text-xs text-muted-foreground), stats (Last 30 Days, +$1.8M green, -$900K red), SVG area chart (h-20) with smooth cubic BÃ©zier curve, linear gradient fill from accent at 15% opacity to transparent, stroke in accent color strokeWidth="1.5"
Accounts card: Header "Accounts" with + and â‹® icons. Three rows (py-3, no dividers, text-xs, justify-between): Credit $98,125.50, Treasury $6,750,200.00, Operations $1,592,864.82
...
No dark mode â€” light only
All colors use semantic Tailwind tokens, never raw color values in components
The SVG chart uses a hand-crafted cubic BÃ©zier path, not a charting library
`````