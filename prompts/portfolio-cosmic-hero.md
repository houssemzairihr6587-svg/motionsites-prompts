...
`````nPrompt to recreate this landing page:
...
### Fonts
Google Fonts import: Inter (300â€“700) and Instrument Serif (italic, 400).
- --font-body: 'Inter', sans-serif â†’ Tailwind font-body
- --font-display: 'Instrument Serif', serif â†’ Tailwind font-display

### CSS Custom Properties (HSL, no hsl() wrapper â€” Tailwind adds it)
--bg: 0 0% 4%;
--surface: 0 0% 8%;
--text: 0 0% 96%;
--muted: 0 0% 53%;
...
### Accent Gradient
linear-gradient(90deg, #89AACC 0%, #4E85BF 100%) â€” used on logo ring, hover borders, progress bars. CSS utility class .accent-gradient.

### Custom Animations (in index.css)
- @keyframes scroll-down â€” translateY(-100%) â†’ translateY(200%), 1.5s ease-in-out infinite
- @keyframes role-fade-in â€” opacity 0 + translateY(8px) â†’ opacity 1 + translateY(0), 0.4s ease-out
- @keyframes gradient-shift â€” background-position 0% 50% â†’ 100% 50% â†’ 0% 50%, 6s ease infinite (for animated gradient borders)
...
Full-screen overlay (fixed inset-0 z-[9999] bg-bg). Uses requestAnimationFrame counter from 000â†’100 over 2700ms.

- Top-left: "Portfolio" label â€” text-xs text-muted uppercase tracking-[0.3em]. Animates y:-20â†’0, opacity 0â†’1.
- Center: Rotating words ["Design", "Create", "Inspire"] cycling every 900ms. AnimatePresence mode="wait" with y:20â†’0â†’-20 transitions. text-4xl md:text-6xl lg:text-7xl font-display italic text-text-primary/80.
- Bottom-right: Counter display â€” text-6xl md:text-8xl lg:text-9xl font-display text-text-primary tabular-nums. Shows String(count).padStart(3, "0").
...
- Uses hls.js â€” if Hls.isSupported(), create HLS instance; else if native HLS support, set video.src directly.
...
1. Logo: 9Ã—9 circle with accent gradient border (reverses direction on hover). Inner bg-bg circle with "JA" in font-display italic text-[13px]. Scales 110% on hover.
2. Divider: w-px h-5 bg-stroke mx-1 (hidden on mobile)
3. Nav links: ["Home", "Work", "Resume"] â€” text-xs sm:text-sm rounded-full px-3 sm:px-4 py-1.5 sm:py-2. Active: text-text-primary bg-stroke/50. Inactive: text-muted hover:text-text-primary hover:bg-stroke/50.
4. Divider
5. "Say hi" button: Same size as nav links. On hover, shows accent gradient border behind (using absolute span with inset: -2px). Inner content wrapped in bg-surface rounded-full backdrop-blur-md. Includes "â†—" arrow.

### Hero Content (centered, z-10)
- Eyebrow: text-xs text-muted uppercase tracking-[0.3em] mb-8 â€” "COLLECTION '26". Class blur-in.
- Name: text-6xl md:text-8xl lg:text-9xl font-display italic leading-[0.9] tracking-tight text-text-primary mb-6 â€” "Michael Smith". Class name-reveal.
- Role line: "A {role} lives in Chicago." â€” roles cycle every 2s through ["Creative", "Fullstack", "Founder", "Scholar"]. Role word uses font-display italic text-text-primary animate-role-fade-in inline-block with key={roleIndex} for re-triggering animation.
- Description: text-sm md:text-base text-muted max-w-md mb-12 â€” "Designing seamless digital interactions by focusing on the unique nuances which bring systems to life."
...
- .name-reveal: opacity 0â†’1, y 50â†’0, duration 1.2s, delay 0.1s
- .blur-in: opacity 0â†’1, filter blur(10px)â†’blur(0px), y 20â†’0, duration 1s, stagger 0.1, delay 0.3s
...
### Header
Framer Motion whileInView â€” opacity 0â†’1, y 30â†’0, duration 1s, ease [0.25,0.1,0.25,1], viewport once margin "-100px".
- Eyebrow: w-8 h-px bg-stroke + "Selected Work" text-xs text-muted uppercase tracking-[0.3em]
- Heading: "Featured *projects*" â€” italic word in font-display italic
- Subtext: "A selection of projects I've worked on, from concept to launch."
- "View all work" button (desktop only, hidden md:inline-flex) â€” rounded-full with gradient hover border ring + right arrow
...
- Halftone overlay: radial-gradient(circle, #000 1px, transparent 1px) at 4Ã—4px, opacity-20 mix-blend-multiply
- Hover: bg-bg/70 opacity-0â†’1 + backdrop-blur-lg
- Hover label: pill with animated gradient border, white bg, "View â€” *Title*" (title in font-display italic)
...
### GSAP Marquee
"BUILDING THE FUTURE â€¢ " repeated 10Ã—. GSAP xPercent: -50, duration 40, ease "none", repeat -1.
