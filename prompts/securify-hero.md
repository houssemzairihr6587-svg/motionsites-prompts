...
`````nBuild a full-screen hero section for a data-security SaaS landing page called "securify" using React + TypeScript + Tailwind CSS, with a looping fullscreen background video, a floating pill-shaped navbar, and large staggered typography.
...
Center pill (hidden on mobile): hidden md:flex items-center gap-1 bg-neutral-900/90 backdrop-blur rounded-full px-3 py-2 with four anchor links: "platform", "solutions", "company", "support" â€” each text-neutral-300 hover:text-white transition-colors text-sm px-5 py-2 rounded-full.
Right button: "get started" â€” bg-white text-black text-sm font-normal rounded-full px-6 py-3 hover:bg-neutral-200 transition-colors.
...
"protect" â€” left-4 md:left-10 top-[18%]
"your" â€” right-4 md:right-10 top-[38%]
"data" â€” left-[18%] md:left-[28%] top-[58%]
All lowercase.
...
Stat block â€” top-right (absolute right-6 md:right-24 top-[14%]):

Row: flex items-center gap-3 justify-end â€” a diagonal divider (hidden md:block h-px w-24 bg-white/40 rotate-[20deg]) then number "+65k" (text-4xl md:text-5xl font-medium tracking-tight).
...
Stat block â€” bottom-left (absolute left-6 md:left-20 bottom-20 md:bottom-24):
...
Stat block â€” bottom-right (absolute right-6 md:right-20 bottom-16 md:bottom-20):
