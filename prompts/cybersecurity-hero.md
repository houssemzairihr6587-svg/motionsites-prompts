...
`````nBuild a **single-page React + TypeScript (Vite)** landing hero for a product called **"Xero"** that recreates the following section exactly. Use the **Inter** Google Font (weights 300, 400, 500, 600, 700, 800). Do not use Tailwind utility classes for the hero â€” write plain CSS in a global stylesheet. No purple/indigo branding outside the specified pink-magenta gradient arc.
...
1. **`<nav>`** â€” sticky-style top bar (not actually sticky, just at top)
2. **`<section class="hero-card">`** â€” the rounded dark hero card with the animated icon pipeline
3. **`<div class="brands">`** â€” a row of 5 monochrome brand logos
...
- **Left**: `<span class="nav-logo">Xero</span>` â€” `font-size: 1.05rem; font-weight: 700; letter-spacing: -0.01em;`
...
- **Right**: `<div class="nav-actions">` containing two pill buttons:
  - `.btn-login` â€” `rgba(255,255,255,0.06)` bg, 1px border `--border`, white text, padding `7px 18px`, `border-radius: 999px`, `font-size: 0.82rem`, `font-weight: 500`. Hover: bg `rgba(255,255,255,0.12)`.
  - `.btn-signup` â€” solid white bg, black `#0a0a0f` text, same dimensions, `font-weight: 600`. Hover: `opacity: 0.88`.
...
- A `.menu-toggle` hamburger appears: 24Ã—14 button with two 2px-tall white spans. When `.active`, span 1 rotates `translateY(6px) rotate(45deg)` and span 2 rotates `translateY(-6px) rotate(-45deg)` to form an X.
...
A radial gradient positioned at `50% -70%` with **many manually-tuned stops** producing a smooth darkâ†’pinkâ†’white arc near the top:
```
background:
...
1. **`<svg class="beam-svg">`** â€” absolutely-positioned over the whole pipeline (`overflow: visible`), containing:
...
2. **Left node** `.icon-node.node-light-right` (id `node-stack`) â€” Lucide-style **layers** SVG (3 stacked diamonds): `<polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 12 17 22 12"/>`.

3. **`.pipeline-line`** â€” `width: 160px; height: 1px;` linear gradient `90deg, rgba(255,255,255,0.15), rgba(255,255,255,0.07)`.

4. **Center wrapper** with `position: relative;` containing:
   - **`.splash`** â€” 100Ã—100 absolutely centered, `border-radius: 50%`, `background: radial-gradient(circle, rgba(255,77,200,0.6) 0%, transparent 70%)`, initial `opacity: 0; transform: scale(0.4); z-index: 2;`
   - **`.icon-node-center`** (id `node-x`) â€” 64Ã—64 round, `background: #1e1e2c`, neumorphic shadow (see below), containing the **Xero "X" logoipsum** SVG (`viewBox="0 0 40 40"`) â€” the multi-cut path provided in the source.

5. **`.pipeline-line.right`** â€” same 160Ã—1 line, gradient reversed.

6. **Right node** `.icon-node.node-light-left` (id `node-shield`) â€” Lucide-style **shield-check** SVG: `<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/>`.

### Side Node Styling
`.icon-node`: 46Ã—46 round, `background: #1a1a24`, `cursor: pointer`, `z-index: 3`, with **neumorphic** shadow stack:
```
box-shadow:
...
Inner SVG: 20Ã—20, stroke `rgba(255,255,255,0.7)`, `stroke-width: 1.5`, fill none, round caps.

### Center Node Styling
`.icon-node-center`: 64Ã—64, `background: #1e1e2c`, similar but stronger neumorphic shadow:
```
...
```
Inner Xero SVG: 28Ã—28, `fill: white`.

### Side-Light Glows
- `.node-light-right::before` â€” half-circle radial glow on the right side: `radial-gradient(circle at right, rgba(200,200,200,0.45) 0%, transparent 70%)`, `opacity: 0` default, `opacity: 1` when `.active` (300ms transition).
- `.node-light-left::before` â€” same but on left, color `rgba(200,100,255,0.5)`.
...
|---|---|---|
| **`p1`** | 800 ms | `percentage` interpolates `0 â†’ 0.5`. While `p < 0.4`, add `.active` to `node-stack`; remove after. At end: switch to `splash`, hide both beam paths (`opacity: 0`), add `.animate` to splash. |
| **`splash`** | 800 ms | Wait. After elapsed: switch to `p2`, remove `.animate`, restore `opacity: 1` on both beam paths. |
| **`p2`** | 800 ms | `percentage` interpolates `0.5 â†’ 1.0`. While `p > 0.6`, add `.active` to `node-shield`. At end: remove `.active`, switch to `idle`. |
| **`idle`** | 1000 ms | Wait, then loop back to `p1`. |

Total cycle â‰ˆ 3.4 seconds, infinite.
...
Five `.brand-item` blocks (each: flex, gap 10, color `rgba(255,255,255,0.35)`, font-size 1.1rem, font-weight 500, white-space nowrap, with a 22Ã—22 SVG):

1. **Expedia** â€” `<circle cx=12 cy=12 r=10 fill=current /><path fill="var(--bg)" d="M8 9h8v2H8zm0 4h6v2H8z"/>` then text `Expedia`.
2. **asana** â€” three filled circles: `(12,7,r=4)`, `(5,16,r=3.5)`, `(19,16,r=3.5)`, text `asana`.
3. **zenefits** â€” three stroked horizontal polylines (lengths 16/8/16) at y=8/12/16, text `zenefits`.
4. **HubSpot** â€” small filled circle `(15.5,8.5,r=2.5)`, stroked circle `(8.5,8.5,r=2)`, paths connecting them; text `HubSp<span class="hubspot-dot"></span>t` where `.hubspot-dot` is a 6Ã—6 round superscript dot.
5. **loom** â€” circle `(12,12,r=9)` plus vertical/horizontal/diagonal stroke lines forming a globe-with-X, text `loom`.

## Responsive Breakpoints

- `â‰¤ 860px`: pipeline `gap: 0; margin-bottom: 40px;` `.pipeline-line { width: 80px }`.
- `â‰¤ 768px`: enable mobile hamburger menu, `.icon-node` shrinks to 38Ã—38, `.icon-node-center` to 52Ã—52, `.hero-card { padding: 60px 20px 60px; min-height: auto }`, `.brands { gap: 32px }`.
- `â‰¤ 480px`: `.hero-card { border-radius: 16px }`, `.brands { gap: 24px }`.

## Z-Index Stack (critical for splash/beam layering)

- `0` â€” gradient arc + grid overlay
- `1` â€” pipeline container, hero text
- `2` â€” beam SVG, splash
- `3` â€” all icon nodes
- `4` â€” node side-light glows
- `1000-1001` â€” mobile nav overlay and toggle
