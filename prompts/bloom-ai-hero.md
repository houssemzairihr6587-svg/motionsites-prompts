...
`````nCreate a full-screen hero landing page for "Bloom" â€” an AI-powered plant/floral design platform. The design uses a liquid glass morphism aesthetic over a looping video background.
...
Fonts
Display/Body: Poppins (Google Fonts) â€” used for headings and body text
Serif accent: Source Serif 4 (Google Fonts) â€” used only for italic/emphasis text inside headings (e.g., <em>, <i>, .italic inside h1-h3)
Headings use font-weight: 500

Color Palette
Strict grayscale only â€” all CSS variables are 0 0% X% HSL values
...
Nav: Logo image (/logo.png, 32Ã—32) + "bloom" text (semibold, 2xl, tracking-tighter, white) on left. "Menu" button with Menu icon on right, liquid-glass pill.
Hero center (flex-1, centered):
Logo image again (80Ã—80)
h1: "Innovating the / spirit of bloom AI" â€” text-6xl lg:text-7xl, tracking-[-0.05em], white. The italic part uses font-serif text-white/80
...
Three pills: "Artistic Gallery", "AI Generation", "3D Structures" â€” liquid-glass, rounded-full, text-xs text-white/80
Bottom quote:
"VISIONARY DESIGN" label (text-xs tracking-widest uppercase text-white/50)
Quote: "We imagined a realm with no ending." â€” mixed font-display/font-serif italic spans
...
Bottom card: flower image thumbnail (from @/assets/hero-flowers.png, 96Ã—64), "Advanced Plant Sculpting" title + description, and a "+" button. All liquid-glass.
...
No border classes anywhere â€” glass effect handles all borders via ::before
