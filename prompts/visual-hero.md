...
`````n**Build a fullscreen hero section in a Vite + React + TypeScript + Tailwind CSS project. Use `gsap` and `lucide-react`. No other UI libraries.**
...
**LogoMark** â€” inline SVG, 44x26, viewBox `0 0 44 26`, three white rects at x=0/16/30, y=3, widths 14/12/14, height 20, rx=3.
...
**Effect 1 â€” Frame capture (boomerang setup):**
...
**Effect 3 â€” Parallax mouse tracking (gsap):**
...
4. **Bottom row:** fixed, `bottom-12 left-0 right-0 px-10 flex items-end justify-between z-20`, fade-in with `transition-all duration-1000 delay-300`.
   ...
   - Center absolute `left-1/2 -translate-x-1/2 bottom-0 flex items-center gap-3` with two buttons:
     ...
     - Secondary: `liquid-glass group text-white text-sm font-body font-medium rounded px-6 py-3 active:scale-[0.97] transition-all duration-200 hover:scale-[1.03] hover:shadow-[inset_0_1px_1px_rgba(255,255,255,0.2),0_0_20px_2px_rgba(255,255,255,0.07)]` â€” label "See templates".
   - Right `<p>`: same classes as left plus `text-right`, text: "Describe what you see in your head â€” get images that actually match."

### Notes
- Tailwind default border-radius is overridden to `9999px` (full pill) â€” every `rounded` in the markup produces pill corners.
- Do NOT use `video.currentTime` to reverse â€” the boomerang uses the captured `frames[]` array only.
