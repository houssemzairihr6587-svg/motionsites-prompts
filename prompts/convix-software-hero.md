...
`````nBuild a fully responsive, full-viewport hero section for a PR-agency SaaS called "Convix Software" with these exact specs:
...
Logo (left, shrink-0): orange #ef4d23 8-petal flower SVG â€” 8 circles at radius 10 around center (16,16) plus center circle, all r=3.5, viewBox 32Ã—32, rendered w-7 h-7 sm:w-8 sm:h-8
...
Badge: inline-flex items-center gap-2 bg-white rounded-full px-4 py-1.5 shadow-sm, 13px â€” orange dot + "Convix Software"
...
CTA button mt-6 sm:mt-8 inline-flex items-center gap-3 bg-[#0b0f1a] text-white rounded-full pl-6 sm:pl-7 pr-2 py-2 sm:py-2.5, 14px: "Get Started" + w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-white/15 containing ChevronRight (4Ã—4)
Dashboard Preview
...
Card 1 â€” Clicks (white, rounded-2xl, p-5)
...
Toggle pill bottom: bg-neutral-100 rounded-full p-1 flex â€” "Impressions" active (white card + shadow) / "Clicks" inactive
Card 2 â€” Form (white, rounded-2xl, p-5, flex flex-col gap-3)
Two label+dropdown groups (label 12px neutral-700, button bordered rounded-lg px-3 py-2 with ChevronDown):
"Show figures for" â†’ "This month"
"Compare period by" â†’ "Month-to-date (MTD)"
Two label+input groups with # prefix:
"Ste targets (This month)" â†’ 10
"Ste targets (This year)" â†’ 100
Footer: orange #ef4d23 "Save" button (rounded-lg px-5 py-2), underlined "Cancel", X icon pushed to right (ml-auto)
Card 3 â€” Video Starts (white, rounded-2xl, p-5)
...
40 tick marks spanning a 180Â° arc (start at angle Ï€, sweep to 2Ï€); active count = round(value/100 * 40)
...
Fully responsive: navbar collapses to hamburger under md, headline/CTA scale via clamp(), dashboard grid steps from 1 â†’ 2 â†’ 3 columns
`````