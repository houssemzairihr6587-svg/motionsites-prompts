...
`````nCreate a full-screen dark hero section with a looping background video, navbar, headline, subtitle, CTA button, and a logo marquee at the bottom. Here are the exact specifications:
...
Center: nav items â€” "Features" (with ChevronDown), "Solutions", "Plans", "Learning" (with ChevronDown). Each is a button with text-foreground/90 and hover transition
...
"AI" uses bg-clip-text text-transparent with backgroundImage: linear-gradient(to left, #6366f1, #a855f7, #fcd34d) (indigo â†’ purple â†’ amber)
Subtitle: "The most powerful AI ever deployed / in talent acquisition" â€” text-hero-sub, text-lg, leading-8, max-w-md, mt-[9px], opacity-80
...
Marquee animation: translateX(0%) â†’ translateX(-50%), 20s linear infinite
...
Section structure: min-h-screen flex flex-col â€” navbar at top, content centered via flex-1 flex items-center justify-center, marquee at bottom.
`````